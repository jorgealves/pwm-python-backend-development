...
from models import Ementa, MenuItem
# 4 clientes
...
def get_menu()->Ementa:
    ...
    return Ementa(produtos=items)

def get_client_menu_choice(menu:Ementa)->dict:
    result = dict()
    nr_items:int = random.randint(1,6)
    while nr_items:
        item_index:int = random.choice(list(menu.produtos))
        item_menu :MenuItem= menu.produtos[item_index]
        result[item_menu.name]=item_menu.price
        nr_items -=1
    return result
